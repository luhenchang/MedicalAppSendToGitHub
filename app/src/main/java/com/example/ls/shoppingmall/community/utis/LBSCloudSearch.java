...
public class LBSCloudSearch {
	...
	private static String ak ="pa07nVcHwG5S4QkRNfCo22l3jdPLSuWn";
		...
		new Thread() {
			public void run() {
				...
				while (count > 0){
					try {
						...
						if (urlConnection.getResponseCode()==200) {
							...
							Message msgTmp = handler.obtainMessage(MedicalMapActivity.MSG_NET_SUCC);
							msgTmp.obj =result;
							msgTmp.sendToTarget();
							...
							Message msgTmp = handler.obtainMessage(MedicalMapActivity.MSG_NET_STATUS_ERROR);
				...
				if ( count <= 0 && handler != null){
					Message msgTmp =  handler.obtainMessage(MedicalMapActivity.MSG_NET_TIMEOUT);
					msgTmp.sendToTarget();
				}
